...
IF "%ERRORLEVEL%"=="0" (
  ...
  ECHO If you don't close Steam, you will be taken staight to the shutdown options.
  TIMEOUT /t 20
...
CHOICE /M "Want to shutdown after a set delay or use Smart Shutdown?"
IF "%ERRORLEVEL%"=="1" (
...
IF "%sh_delay_active%"=="y" (
  ...
  CHOICE /M "Would you like to enable Smart Shutdown? The computer will shutdown when it has detected that Steam isn't downloading any updates for games."
  IF "!ERRORLEVEL!"=="1" (
  ...
  CHOICE /C rsh /M "Select Shutdown mode: Restart (r) Shutdown (s) Hibernate/Sleep (h): "
...
IF "%sh_delay_active%" EQU "y" (
  ...
  IF "!smart_shutdown_enabled!" EQU "y" (
    ...
    FOR /f "tokens=2 delims=," %%c in ('typeperf "\Process(Steam)\IO Data Operations/sec" -si 1 -sc 1 ^| find /V "\\"') do (
      ...
      PING www.duckduckgo.com -n 1 -w 1000 > NUL
